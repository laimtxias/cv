...
\sectionrule
\justifying
Ingénieure diplômée de l'IMT Mines Albi (Data \& Systèmes d'Information), spécialisée en IA générative et machine learning, avec un réflexe marqué d'orientation résultats. À l'aise en environnement international. En recherche d'un poste à partir de septembre 2026.
...
\cvexp{Stagiaire Ingénieure IA Générative}{ALTEN Lab}{Grenoble, France}{Février 2026 -- Août 2026 (6 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Cycle complet d'un projet d'IA générative, de l'état de l'art à la mise en production, en lien direct avec l'équipe métier pour cadrer le besoin et prioriser le périmètre.
    \item Approche itérative (benchmark d'outils, choix argumenté, prototypage rapide) : développement de deux moteurs RAG complémentaires (frameworkless, LangChain/\allowbreak LangGraph), conçus pour être adaptables à différents métiers.
    \item Déploiement en production (API FastAPI conteneurisée, CI/CD) et évaluation quantifiée de l'impact (RAGAS, recall@k).
  \end{itemize}
}

\cvexp{Stagiaire Assistante Ingénieure R\&D}{Yooz}{Aimargues, France}{Avril 2025 -- Août 2025 (4 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Étude comparative quantifiée de plusieurs approches IA (RAG optimisé : 81\% de correspondance vs LLM pur : 64\%), pour orienter le choix de solution selon le profil et les contraintes de chaque client.
    \item Collaboration avec des experts métier comptables de plusieurs entreprises clientes pour cadrer le besoin et valider les résultats.
    \item Développement d'une application de visualisation (Dash) pour objectiver et communiquer les sources d'erreur d'un système existant auprès des équipes.
  \end{itemize}
}
...
\cvproject{Prévision en temps réel du temps d'attente à quai de la SNCF}{2025}{Nettoyage, traitement et agrégation de données de flux réelles. Entraînement de modèles de régression et de séries temporelles en Python, à visée opérationnelle.}
...
\sidebarline{IA Générative \& ML}{RAG, embeddings, LlamaIndex, LangChain/\allowbreak LangGraph, CNN, MLP, classification, clustering}
\sidebarline{Langages}{Python (NumPy, Pandas, Scikit-learn, TensorFlow/\allowbreak PyTorch), R, SQL, C++, Matlab}
\sidebarline{Outils \& déploiement}{Git, Docker, FastAPI, CI/CD}

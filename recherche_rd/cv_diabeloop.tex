...
\sectionrule
\justifying
Ingénieure diplômée de l'IMT Mines Albi (Data \& Systèmes d'Information), avec une solide formation en mathématiques appliquées et une expérience en machine learning et modélisation prédictive sur données réelles bruitées. À l'aise avec les outils LLM pour accélérer la recherche et le développement. En recherche d'un poste de chercheuse / data scientist à partir de septembre 2026.
...
\cvexp{Stagiaire Ingénieure IA Générative}{ALTEN Lab}{Grenoble, France}{Février 2026 -- Août 2026 (6 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Conception et prototypage en Python d'algorithmes de recherche documentaire (RAG), avec benchmark et protocole de test reproductible (métriques quantifiées).
    \item Utilisation experte d'outils LLM (Anthropic, OpenAI, Mistral) pour accélérer la recherche, le prototypage et l'écriture de code.
    \item Collaboration avec les équipes métier pour le transfert d'un prototype de recherche vers une solution en production (API FastAPI conteneurisée, CI/CD).
  \end{itemize}
}

\cvexp{Stagiaire Assistante Ingénieure R\&D}{Yooz}{Aimargues, France}{Avril 2025 -- Août 2025 (4 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Data science sur données réelles non annotées et bruitées (factures) : entraînement et comparaison de modèles de classification supervisée (Random Forest, XGBoost).
    \item Conception et évaluation quantifiée d'algorithmes de recherche sémantique et de RAG pour l'automatisation d'une décision métier (81\% vs 64\% de correspondance selon l'approche).
    \item Clustering et réduction de dimension (PCA, t-SNE, UMAP) pour analyser et diagnostiquer un espace vectoriel en production.
  \end{itemize}
}
...
\cvproject{CNN pour la reconnaissance de chiffres manuscrits}{2024}{Développement d'un CNN pour la classification d'images (Dataset MNIST) avec PyTorch : convolution, pooling, rétropropagation et optimisation (Adam).}
...
\sidebarline{Languages}{Python (NumPy, Pandas, Scikit-learn, TensorFlow/\allowbreak PyTorch), C++, R, SQL, Matlab}
\sidebarline{Machine Learning}{Régression, séries temporelles, classification, clustering, CNN, MLP}
\sidebarline{Outils \& LLM}{Git, Docker, FastAPI, API LLM (Anthropic, OpenAI, Mistral)}

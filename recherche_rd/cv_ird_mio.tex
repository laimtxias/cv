...
\newcommand{\sectionrule}{\vspace{-6pt}{\color{accent}\hrule height 1pt}\vspace{4pt}}
...
\sectionrule
\justifying
Ingénieure diplômée de l'IMT Mines Albi (Data \& Systèmes d'Information), avec une expérience en conception de pipelines reproductibles pour l'intégration et l'analyse de données hétérogènes, bases de données relationnelles et méthodes statistiques multivariées (réduction de dimension, machine learning). Anglais scientifique courant (C1). En recherche d'un poste d'ingénieure de recherche à partir de septembre 2026.
...
\cvexp{Stagiaire Ingénieure IA Générative}{ALTEN Lab}{Grenoble, France}{Février 2026 -- Août 2026 (6 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Conception de pipelines reproductibles pour l'intégration de données hétérogènes (documents, images, tableaux), avec protocole de test et métriques quantifiées (WER, CER, NED, GriTS, F1).
    \item Structuration et harmonisation de données sur base relationnelle (PostgreSQL/\allowbreak pgvector) ; développement de deux moteurs de traitement complémentaires (frameworkless, LangChain/\allowbreak LangGraph).
    \item Documentation et diffusion des outils développés (API FastAPI conteneurisée, CI/CD), déploiement en production.
  \end{itemize}
}

\cvexp{Stagiaire Assistante Ingénieure R\&D}{Yooz}{Aimargues, France}{Avril 2025 -- Août 2025 (4 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Développement de modèles de machine learning (Random Forest, XGBoost) et de méthodes statistiques de réduction de dimension (PCA, t-SNE, UMAP) sur données réelles hétérogènes.
    \item Intégration de données multimodales (texte, embeddings, métadonnées) dans un système de recherche sémantique en production.
    \item Étude comparative quantifiée de plusieurs approches méthodologiques (81\% vs 64\% de correspondance), documentée pour orienter la décision selon le contexte d'usage.
  \end{itemize}
}
...
\cvproject{Prévision en temps réel du temps d'attente à quai de la SNCF}{2025}{Nettoyage et modélisation de données de flux (régression, séries temporelles) en Python.}
...
\sidebarline{Programmation scientifique}{Python (NumPy, Pandas, Scikit-learn, TensorFlow/\allowbreak PyTorch), R, C++, Matlab, SQL}
\sidebarline{Bases de données}{SQL, PostgreSQL, modélisation de schémas relationnels}
\sidebarline{Méthodes statistiques \& ML}{Réduction de dimension (PCA, t-SNE, UMAP), régression, classification, clustering, CNN, MLP}
\sidebarline{Développement \& reproductibilité}{Git, Docker, CI/CD, FastAPI, protocoles de test reproductibles}

...
\begin{center}
  ...
  {\color{accent}\itshape\large Ingénieure Data Science \& IA Générative -- IMT Mines Albi}\\[6pt]
  {\small
    Grenoble, France
    \,\textbullet\,
...
\sectionrule
\justifying
Ingénieure diplômée de l'IMT Mines Albi (Data \& Systèmes d'Information), avec une expérience concrète en traitement de données textuelles non structurées, recherche sémantique et évaluation de LLM (RAG, RAGAS, recall@k, MRR) en production. En recherche d'un poste d'ingénieure data science à partir de septembre 2026.
...
\cvexp{Stagiaire Ingénieure IA Générative}{ALTEN Lab}{Grenoble, France}{Février 2026 -- Août 2026 (6 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Conception et développement d'un service RAG multimodal as-a-service pour la recherche documentaire industrielle : API backend FastAPI conteneurisée (Docker), pensée comme abstraction réutilisable et adaptable à différents métiers.
    \item Orchestration de LLM, deux moteurs RAG complémentaires (frameworkless, LangChain/\allowbreak LangGraph) ; ingestion (chunking, embeddings), recherche hybride BM25/sémantique (fusion RRF, PostgreSQL : pgvector + ParadeDB).
...
\cvexp{Stagiaire Assistante Ingénieure R\&D}{Yooz}{Aimargues, France}{Avril 2025 -- Août 2025 (4 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Application web (Dash, Plotly) de visualisation d'embeddings pour analyser des données textuelles non structurées (factures) et catégoriser les erreurs d'un moteur de recherche sémantique (Milvus).
    \item Système RAG (LlamaIndex, ChromaDB, GPT-4o) indexé sur le Plan Comptable Général pour affecter automatiquement des comptes comptables à des descriptions de produits sans données annotées (81\% de précision).
    \item Comparaison de modèles de classification ML (XGBoost, meilleurs résultats) pour réduire les faux positifs de l'affectation comptable.
  \end{itemize}
}
...
\sidebarline{NLP / IA générative}{RAG, LLM (orchestration, évaluation), embeddings, recherche sémantique \& hybride, LangChain, LangGraph, LlamaIndex}
\sidebarline{Backend \& Infrastructure}{FastAPI, architectures d'intégration, Docker, CI/CD, Git/GitLab}

...
\sectionrule
\justifying
Ingénieure diplômée de l'IMT Mines Albi (Data \& Systèmes d'Information), avec une expérience en conception de pipelines de traitement de données (ingestion, indexation, bases NoSQL et moteurs de recherche : OpenSearch, ChromaDB, Milvus, PostgreSQL/pgvector) et en déploiement d'API conteneurisées (Docker, CI/CD). En recherche d'un poste d'ingénieure Data Ops à partir de septembre 2026.
...
\cvexp{Stagiaire Ingénieure IA Générative}{ALTEN Lab}{Grenoble, France}{Février 2026 -- Août 2026 (6 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Conception d'un pipeline d'ingestion et d'indexation de données (chunking, embeddings) sur bases vectorielles et moteurs de recherche : PostgreSQL/\allowbreak pgvector, OpenSearch.
    \item Développement de deux moteurs de traitement de données complémentaires (frameworkless, LangChain/\allowbreak LangGraph), exposés via une API FastAPI conteneurisée (Docker) et intégrés en production.
    \item Benchmark d'outils d'extraction de documents et protocole de test reproductible (métriques quantifiées), CI/CD, déploiement en production sur Open WebUI.
  \end{itemize}
}

\cvexp{Stagiaire Assistante Ingénieure R\&D}{Yooz}{Aimargues, France}{Avril 2025 -- Août 2025 (4 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Intégration de bases de données vectorielles NoSQL (Milvus, ChromaDB) dans un pipeline de recherche sémantique en production, avec GitLab pour la gestion de version.
    \item Application web (Dash) de visualisation des flux de données et d'expérimentation de solutions IA (recherche sémantique, RAG, LLM) pour la gestion comptable.
    \item Data science sur données réelles non annotées : entraînement et comparaison de modèles de classification supervisée (Random Forest, XGBoost).
  \end{itemize}
}
...
\cvproject{Prévision en temps réel du temps d'attente à quai de la SNCF}{2025}{Nettoyage, traitement et agrégation de données de flux en temps réel. Entraînement de modèles de régression et de séries temporelles en Python.}
...
\sidebarline{Data \& NoSQL}{OpenSearch, ChromaDB, Milvus, PostgreSQL/\allowbreak pgvector}
\sidebarline{Languages}{Python (NumPy, Pandas, Scikit-learn, TensorFlow/\allowbreak PyTorch), SQL, R, C++, Matlab}
\sidebarline{DevOps \& Outils}{Git, GitLab, Docker, CI/CD, FastAPI}
\sidebarline{Machine Learning}{Régression, séries temporelles, classification, clustering, CNN, MLP}

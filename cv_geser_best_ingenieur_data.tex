...
\begin{center}
  ...
  {\color{accent}\itshape\large Ingénieure Data \& Machine Learning -- IMT Mines Albi}\\[6pt]
  {\small
    Grenoble, France
    \,\textbullet\,
...
\sectionrule
\justifying
Ingénieure diplômée de l'IMT Mines Albi (Data \& Systèmes d'Information), avec une expérience concrète de développement en production : pipelines de données, machine learning (classification, clustering, vision par CNN) et déploiement (FastAPI, Docker, CI/CD). En recherche d'un poste d'ingénieure data à partir de septembre 2026.
...
\cvexp{Stagiaire Ingénieure IA Générative}{ALTEN Lab}{Grenoble, France}{Février 2026 -- Août 2026 (6 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Développement d'un service backend FastAPI conteneurisé (Docker) exposant un pipeline d'IA (RAG multimodal), conçu comme brique logicielle réutilisable et intégrable à différents systèmes.
    \item Ingestion et traitement de données (chunking, embeddings), recherche hybride lexicale/sémantique sur bases vectorielles (PostgreSQL : pgvector + ParadeDB).
    \item Tests, évaluation quantifiée sur benchmark académique (recall@k, MRR, bootstrap), CI/CD et déploiement en production.
  \end{itemize}
}

\cvexp{Stagiaire Assistante Ingénieure R\&D}{Yooz}{Aimargues, France}{Avril 2025 -- Août 2025 (4 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Entraînement et comparaison de plusieurs modèles de classification supervisée (XGBoost, meilleurs résultats) à partir de caractéristiques extraites de données réelles, pour réduire les faux positifs.
    \item Clustering (K-Means) et réduction de dimension (PCA, t-SNE, UMAP) sur un jeu de données réel (11508 lignes) : application web (Dash, Plotly) pour analyser et catégoriser les erreurs d'un moteur de recherche sémantique (Milvus).
    \item Système de classification automatique de texte (RAG) sur données non annotées, avec mesure quantifiée de la performance (81\% de précision).
  \end{itemize}
}
...
\cvproject{CNN pour la reconnaissance de chiffres manuscrits}{2025}{Développement d'un réseau de neurones convolutif (CNN) pour la classification d'images (Dataset MNIST) avec PyTorch : couches de convolution/pooling, fonctions d'activation, rétropropagation, optimisation (Adam) et régularisation (dropout).}
...
\sidebarline{Langages}{Python (FastAPI, Pandas, NumPy, Scikit-learn, PyTorch), SQL}
\sidebarline{Machine Learning}{Classification (supervisé), clustering (non supervisé), réduction de dimension, vision par CNN, séries temporelles}
\sidebarline{Backend \& Infrastructure}{FastAPI, Docker, CI/CD, Git, architectures d'intégration}
\sidebarline{Bases de données}{SQL, BM25 (ParadeDB), bases vectorielles (pgvector, Milvus, ChromaDB)}
\sidebarline{IA générative / LLM}{RAG, orchestration de LLM, agents IA, LangChain, LangGraph, LlamaIndex, embeddings}

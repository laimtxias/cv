...
\sectionrule
\justifying
Ingénieure diplômée de l'IMT Mines Albi, option Génie des systèmes d'information. En recherche d'un poste d'ingénieure en Data et Machine Learning à partir de septembre 2026.
...
\cvexp{Stagiaire Ingénieure IA Générative}{ALTEN Lab}{Grenoble, France}{Février 2026 -- Août 2026 (6 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Cycle complet du projet (état de l'art, recueil du besoin métier, traitement des données, mise en production) d'une solution RAG multimodale pour la recherche documentaire industrielle, conçue pour être adaptable à différents métiers.
    \item Benchmark d'outils d'extraction PDF (Docling, PyMuPDF, OCR, VLM), pipeline d'ingestion (chunking, embeddings, pgvector) et deux moteurs RAG complémentaires (frameworkless, LangChain/\allowbreak LangGraph).
    \item Évaluation complète (RAGAS, recall@k), API FastAPI conteneurisée (Docker), CI/CD, déploiement en production sur Open WebUI.
  \end{itemize}
}

\cvexp{Stagiaire Assistante Ingénieure R\&D}{Yooz}{Aimargues, France}{Avril 2025 -- Août 2025 (4 mois)}{%
  \begin{itemize}[leftmargin=12pt, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Application web (Dash) de visualisation et d'expérimentation de solutions IA (recherche sémantique, RAG, LLM) pour la gestion comptable.
    \item Clustering et réduction de dimension (PCA, t-SNE, UMAP) de l'espace vectoriel pour améliorer la recherche sémantique et l'aide à la décision.
    \item Agents « comptable » connectés à un RAG indexé sur le Plan Comptable Général, exposés via un serveur MCP. (Sentence Transformers, LlamaIndex, Milvus, ChromaDB)
  \end{itemize}
}
...
\cvedu{IMT Mines Albi}{Ingénieur Généraliste}{Albi, France}{2023 -- 2026}{Optimisation, Base de données, Calcul numérique, Big Data, Machine Learning, Science des matériaux, Thermodynamique, Mécanique des fluides.}
...
\cvproject{Thermométrie par photoluminescence dans le domaine cryogénique (projet de recherche)}{2024}{Recherche bibliographique, plan expérimental, rapport et poster scientifique sur l'étude de luminophores pour la mesure de température à très basse température.}
...
\sidebarline{Outils}{Git, Docker, FastAPI}
\sidebarline{NLP / LLM}{RAG, embeddings, LlamaIndex, LangChain, LangGraph}
\sidebarline{Deep / Machine Learning}{CNN, MLP, régression, séries temporelles, classification, clustering}
